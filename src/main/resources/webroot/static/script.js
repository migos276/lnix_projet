      ...
      if (data.creators && data.creators.length > 0) {
        creatorCards.innerHTML = data.creators.map(creator => `
          <div class="creator-card" data-creator-id="${creator.id}" onclick="loadCreatorProfile(${creator.id})">
            <img src="${creator.avatar_url || '/static/default-avatar.jpg'}" 
                 alt="${creator.nom}"
...
function loadCreatorProfile(creatorId) {
  ...
  fetch(`/api/creator/${creatorId}`)
    ...
    .then(creator => {
      ...
      if (profileImage) profileImage.src = creator.avatar_url || '/static/default-avatar.jpg';
      if (creatorName) creatorName.textContent = creator.nom;
      if (creatorRole) creatorRole.textContent = `Rôle: ${creator.role}`;
      
      // Update member since (you might want to add this field to your database)
...
function loadCreatorTemplates(creatorId) {
  fetch(`/api/templates/${creatorId}`)
    ...
    .then(data => {
      ...
      if (templatesCount) {
        templatesCount.textContent = currentTemplates.length;
      }
...
function getMediaElement(template) {
  ...
  switch (template.type) {
    ...
    case 'audio':
      ...
      `;
    default:
      return `<img src="${fileUrl}" alt="${template.nom}" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='/static/default-image.jpg'">`;
  }
}
        ...
        testimonialsGrid.innerHTML = data.testimonials.map(testimonial => `
          <div class="testimonial-card" data-id="${testimonial.id}">
            <div class="testimonial-content">
              ...
              ${testimonial.fichier_url ? `<small class="file-info"><i class="fas fa-file"></i> ${testimonial.fichier_url}</small>` : ''}
