...
public class MainVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    String dbPort = System.getenv().getOrDefault("DB_PORT", "5432");  // PostgreSQL utilise 5432
    String dbName = System.getenv().getOrDefault("DB_NAME", "linx");
    String dbUser = System.getenv().getOrDefault("DB_USER", "root2");
    String dbPassword = System.getenv().getOrDefault("DB_PASSWORD", "Clint.mariadb");
    
    JsonObject config = new JsonObject()
      .put("url", String.format("jdbc:postgresql://%s:%s/%s?ssl=false", 
    ...
    router.route("/static/*").handler(StaticHandler.create("src/main/resources/webroot/static"));
  ...
  private void handleDashboard(RoutingContext ctx) {
    ...
    jdbcClient.query("SELECT id, nom, role, avatar_url FROM users WHERE role='ingenieur'", res -> {
      if (res.succeeded()) {
        ...
        // Get testimonials
        jdbcClient.query("SELECT * FROM temoignages ORDER BY date_creation DESC LIMIT 10", testimonialsRes -> {
          if (testimonialsRes.succeeded()) {
            data.put("testimonials", testimonialsRes.result().getRows());
          }
          
          // Get partners
          jdbcClient.query("SELECT * FROM partenaires ORDER BY date_ajout DESC", partnersRes -> {
  ...
  private void handleGetCreator(RoutingContext ctx) {
    ...
    jdbcClient.queryWithParams(
      "SELECT id, nom, role, avatar_url FROM users WHERE id = ? AND role='ingenieur'",
  ...
  private void handleGetTemplatesByCreator(RoutingContext ctx) {
    ...
    jdbcClient.queryWithParams(
      "SELECT * FROM templates WHERE cree_par = ? ORDER BY date_creation DESC",
      new JsonArray().add(creatorId),
      res -> {
        if (res.succeeded()) {
          ctx.response()
            .putHeader("Content-Type", "application/json")
  ...
  private void handleGetTestimonials(RoutingContext ctx) {
    jdbcClient.query("SELECT * FROM temoignages ORDER BY date_creation DESC", res -> {
      if (res.succeeded()) {
        ctx.response()
          .putHeader("Content-Type", "application/json")
  ...
  private void handleAddTestimonial(RoutingContext ctx) {
    try {
      ...
      params.add(authorId != null ? Integer.parseInt(authorId) : null);
  ...
  private void handleGetPartners(RoutingContext ctx) {
    jdbcClient.query("SELECT * FROM partenaires ORDER BY date_ajout DESC", res -> {
      if (res.succeeded()) {
        ctx.response()
          .putHeader("Content-Type", "application/json")
    ...
    try {
      ...
      vertx.fileSystem().move(upload.uploadedFileName(), finalPath, moveRes -> {
        if (moveRes.succeeded()) {
          // Save to database
          jdbcClient.updateWithParams(
            "INSERT INTO partenaires (nom, logo_url) VALUES (?, ?)",
            new JsonArray().add(name).add(uniqueFilename),
            dbRes -> {
              if (dbRes.succeeded()) {
                ctx.response()
                  .putHeader("Content-Type", "application/json")
  ...
  private void handleLogin(RoutingContext ctx) {
    // ✅ Correct : utiliser ctx.getBody() au lieu de ctx.request().getBody()
    io.vertx.core.buffer.Buffer buffer = ctx.getBody();
    JsonObject body = buffer != null ? buffer.toJsonObject() : null;
    ...
    // Votre logique de traitement du login ici
